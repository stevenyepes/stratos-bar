...
```bash
sudo apt install ./stratos-bar_0.2.0_amd64.deb
```

### Fedora / RHEL
Download the `.rpm` package and install:
```bash
sudo dnf install ./stratos-bar-0.2.0-1.x86_64.rpm
```

### AppImage (Universal)
Download the `.AppImage` file, make it executable, and run:
```bash
chmod +x stratos-bar_0.2.0_amd64.AppImage
./stratos-bar_0.2.0_amd64.AppImage
```
...
bind = $mainMod, SPACE, exec, /path/to/stratos-bar_0.2.0_amd64.AppImage
```
